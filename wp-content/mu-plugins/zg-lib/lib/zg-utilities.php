...
//			}
	 ...
	 * @return mixed
	 */
	function array_search_assoc( $key, $value, $haystack, $strict = false ) {
	...
	/**
	 * This function adds the WP_Meta_Webform prefix for our ZG meta.
	 *
	 * @param int id - Post ID
	 * @param type $common_exclude
	 * @return type
	 */
	function zg_get_post_meta( $id, $meta_key, $single = true ) {
		return get_post_meta( $id, WP_Meta_Webform::META_PREFIX . $meta_key, $single );
	}
...
function zg_get_post_type_archive_link( $post_type ) {
	if ( 'post' == $post_type ) {
 ...
 */
function add_admin_menu_separator( $position = 25 ) {
	global $menu;
	...
	foreach($menu as $offset => $section) {
		...
		if ( $offset === $position ) {
			array_insert_after( $menu, $position, array( array( '', 'read', "separator{$index}", '', 'wp-menu-separator' ) ) );
			break;
