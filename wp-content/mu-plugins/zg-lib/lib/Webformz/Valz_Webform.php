<?php
	 ...
	 * @changelog
	 * 0.6:
	 * - Added radio field type support in add_field()
	 * - fixed minor issue - Notice being created for number input fields
	 * V0.5:
...
// 							if ($fieldValue && !$foundMatch)) $fieldContent .= qq {<option value="" selected>? "$fieldValue"</option>} ;
				...
				case self::RADIO_BUTTON_TYPE:
					if ($readonly){
						...
						$classTag = $this->generate_class_attribute(self::SELECT_CONTROL_CLASS, $fieldParms['class']);
						$fieldContent = '<span id="' . $fieldParms['id'] . '">';
						...
						if ($fieldParms['items']){
							foreach ($fieldParms['items'] as $item) {
								...
								if ($value === $fieldValue) $checked = " checked";

								$fieldContent .= '&nbsp;&nbsp;<label><input name="' . $fieldParms['name'] . '" type="radio" value="' . $value . '"' . $checked . '/>&nbsp;' . $label . '</label>';
							}
						}
						else {
							$fieldContent .= '<option value="" selected>No Data</option>';
